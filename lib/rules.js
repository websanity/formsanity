...
export function checkRule(rule, field, ctx, trigger) {
	...
	switch (rule.kind) {
		...
		case 'group-unique-values': {
			// Only checked on commit (blur, or the change a select or picker
			// fires): while either field is still being typed, a transient
			// match shouldn't flag — only a completed, deliberate duplicate.
