...
function rulesFor(controls) {
	...
	// Conditional requiredness sits on any member of a set, like the selection counts. Native required wins: a field any of whose controls carries it is always required, so the expression would be dead, and the author is told once.
	const conditional = controls.find((c) => c.hasAttribute('data-fs-required'));
	if (conditional) {
		if (controls.some((c) => c.required)) {
			console.warn(`FormSanity: data-fs-required on "${first.name}" is ignored because the field carries required`);
		} else {
