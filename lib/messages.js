...
// Format hints for the format-shaped types, applied as placeholders when the
// author wrote none. Types whose catalog message is a description rather
// than a shape have no hint on purpose.
export const hints = {
	'cvv': '###',
	'us-phone': '(###) ###-####',
...
// A time-of-day bound quoted in a message speaks the user's locale — the
// same presentation the temporal inputs themselves render — never the raw
// 24-hour attribute value.
...
export function messageFor(code, params = {}) {
	// An author-supplied message (data-fs-constraint-message) wins over the
	// catalog: the catalog cannot know what an arbitrary expression means.
