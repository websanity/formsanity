...
function setup(form) {
	...
	// valid() in a relevance expression reads verdicts, and the first
	// relevance pass ran before any verdict existed — so settle to a fixed
	// point: each relevance pass reads the latest verdicts, each verdict
	// pass reads the latest relevance, until neither moves. One round covers
	// the common prefilled form; the chained case (an invalid prefill whose
	// forced-valid first verdict opened a region) needs the second. The cap
	// only guards a cyclic model from oscillating forever.
	...
	// change fires only on a commit (blur, Enter, or a native picker
	// selection), never per keystroke, so it is the moment a valid value gets
	// rewritten to its canonical format — before the refresh, so bounds and
	// dependent fields read the rewritten value. The commit also carries its
	// own trigger: a picker selection must present errors like a blur would,
	// because no blur ever follows it.
	...
	function refreshFor(target, trigger) {
		...
		// The changed field's own verdict settles first so a dependent's
		// valid() reads the new verdict, not last event's.
