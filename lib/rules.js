...
// a comparable number. min/max attributes on a control of one of these types
// compare in this order — the native attributes, extended past the native
// types. An unparsed value is no verdict: the type check owns malformedness.
const ORDERED_TYPES = {
	'duration': (value) => {
	...
	'us-dollar': (value) => {
		...
		return /^\d+(\.\d{2})?$/.test(cleaned) ? Number(cleaned) : null;
	}
};
