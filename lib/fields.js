...
import { checkRule, checkGroups, COMMIT_TRIGGERS } from './rules.js';

export const RANK = { invalid: 0, incomplete: 1, valid: 2 };

// Codes that mean "not answered yet" rather than "answered wrong". The asterisk indicator and the status line carry these; they never bubble on their own — only a submit attempt reveals them as messages.
export const REQUIREDNESS = new Set(['required', 'group.required-any', 'group.required-together', 'min-selected']);

// The temporal inputs display in the user's locale — clock convention and date ordering are the OS's choice, not the author's — so a bound quoted in a bubble must speak the same presentation. The raw attribute values are ISO-shaped and would contradict the field's own face.
const BOUND_FORMATS = {
...
function nativeBoundParam(control, code) {
	...
	// Parsed field by field: handing a date-only string to the Date constructor reads it as UTC midnight, which shifts a day in most timezones.
...
export function createFieldController(field, ctx) {
	...
	const controller = { field, verdict: 'valid', code: null, params: {}, refresh, present, rejectFromServer };
	...
	function compute(trigger) {
		...
		let params = {};
		if (field.set) {
			// A choice group's requiredness is derived here: the platform counts a disabled checked radio as an answer and bars a disabled first control from constraint validation, so ValidityState cannot read relevant members only. No other native constraint applies to a checkbox or a radio.
			if (field.controls.some((c) => c.required) && ctx.valueOf(field.name) === '') {
				verdict = 'incomplete';
				code = 'required';
			}
		} else {
	...
	function refresh(trigger) {
		...
		// Missing means an unanswered obligation — the asterisk's meaning. A wrong answer is not missing: it gets the bubble, never the mark.
		...
		} else if (verdict === 'invalid' || state.shown || (COMMIT_TRIGGERS.has(trigger) && isErrorWorthy)) {
			present();
		}
	...
	// A server rejection lands as a real invalid verdict, so the standing invalid line and the gate follow from the ordinary accounting. It holds only until the next refresh recomputes: editing the field wipes a server verdict the client cannot re-derive, and the resubmit is where the server gets its next say.
	function rejectFromServer(code, message) {
		controller.verdict = 'invalid';
		controller.code = code;
		controller.params = {};
		controller.missing = false;
		state.shown = true;
		ctx.applyRowState(field, 'invalid', false);
