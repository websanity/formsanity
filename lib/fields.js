...
export function createFieldController(field, ctx) {
	...
	// A server rejection lands as a real invalid verdict, so the standing
	// invalid line and the gate follow from the ordinary accounting. It
	// holds only until the next refresh recomputes: editing the field wipes
	// a server verdict the client cannot re-derive, and the resubmit is
	// where the server gets its next say.
