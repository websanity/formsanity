...
import { attachSubmit, attachUnique } from './submit.js';
...
function setup(form) {
	...
	function applyAggregateRowState(field, verdict) {
		const siblings = field.row ? rowFields.get(field.row) : null;
		if (!siblings || siblings.length < 2) { applyRowState(field, verdict); return; }
		let worst = verdict;
		for (const name of siblings) {
			if (name === field.name) continue;
			const sibling = controllers.get(name);
			if (sibling && RANK[sibling.verdict] < RANK[worst]) worst = sibling.verdict;
		}
		applyRowState(field, worst);
	}
