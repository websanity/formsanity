...
// A picker cap presses only where a picker exists, and the wiring probes for one: showPicker without user activation refuses with NotAllowedError where the picker exists, and with NotSupportedError where the browser has none for the type (Safari and Firefox time) — there the cap demotes to an informational glyph, marked fs-inert, keeping its shape but dropping the accent and the pointer. The click handler keeps its own catch for a browser whose probe misreports: the press focuses the control, and a NotSupportedError demotes the cap from then on.
function wirePicker(picker, control) {
	picker.setAttribute('aria-hidden', 'true');
	const demote = () => picker.classList.add('fs-inert');
	try {
		control.showPicker();
	} catch (error) {
		if (error.name === 'NotSupportedError') demote();
	}
...
// A cols group with no authored col-break balances at its midpoint: the engine marks the middle row and the stylesheet's split machinery treats it like a manual break. Count-based, computed once at init — rows later hidden by relevance can leave the columns visually uneven.
