...
function setup(form) {
	...
	// valid() in a relevance expression reads verdicts, and the first relevance pass ran before any verdict existed — so settle to a fixed point: each relevance pass reads the latest verdicts, each verdict pass reads the latest relevance, until neither moves. One round covers the common prefilled form; the chained case (an invalid prefill whose forced-valid first verdict opened a region) needs the second. The cap only guards a cyclic model from oscillating forever.
	for (let round = 0; round < 10 && relevance.refresh(); round++) {
		for (const controller of controllers.values()) {
			controller.refresh();
		}
	}
