...
// The triggers that mean a value was committed — blur, or a change event's picker selection — as opposed to a per-keystroke input.
export const COMMIT_TRIGGERS = new Set(['blur', 'change']);
...
export function checkRule(rule, field, ctx, trigger) {
	...
	switch (rule.kind) {
		...
		case 'max-file-size': {
			// Every selected file counts, not just the first: a [multiple] input whose second file blows the cap is just as unacceptable as one whose first does.
		...
		case 'required': {
			// Conditional requiredness reads the way relevance reads: two values, and an unanswered reference is ''. The author picks the polarity with the expression, so there is no skip for empty references as a constraint has.
			if (value !== '') return null;
			const compiled = compiledExpression(rule.param);
			// A malformed expression compiles to an always-true fallback, which is inert for relevance and constraints but would make this field permanently required, so the rule steps aside and the compile error already reported stands alone.
			if (compiled.error) return null;
			if (!compiled.evaluate(expressionCtx(ctx))) return null;
			return { verdict: 'incomplete', code: 'required', params: {} };
		}
		case 'constraint': {
			if (value === '') return null;
			const compiled = compiledExpression(rule.param);
			// A constraint judges answers, not absence: while any referenced field is unanswered — or holds an answer its own validation rejects — there is nothing to judge yet. The empty-is-false polarity must not flag this field for a question nobody has answered, and an invalid reference carries its own error, so flagging the host too would point at the wrong field.
			for (const dep of compiled.deps) {
				if (dep === field.name) continue;
				if (ctx.valueOf(dep) === '' || ctx.isFieldValid?.(dep) === false) return null;
			}
			const verdict = compiled.verdict(expressionCtx(ctx));
			if (verdict === 'satisfied') return null;
			// A dead-end (only == can prove one: neither value a prefix of the other) is invalid and presents immediately — the confirm field flags the moment it diverges from its target.
			return { verdict: verdict === 'dead-end' ? 'invalid' : 'incomplete', code: 'constraint', params: { message: rule.message } };
		}
		case 'min-time':
		case 'max-time': {
			if (value === '') return null;
			// The time-of-day component of a datetime-local value; zero-padded HH:MM compares correctly as a string.
			...
			if (min !== undefined && max !== undefined && min > max) {
				// A reversed window wraps midnight, as native time bounds do. One rule speaks for the pair so the violation reports once, and it is incomplete: the value can still climb into the evening window.
		...
		case 'min-selected':
		case 'max-selected': {
			// The count reads the field the way its value does, so a disabled member or option is out of play. A field that is neither a set nor a list has no count, and the rule does not apply to it.
			const [first] = field.controls;
			if (!field.set && !first.multiple) return null;
			const selected = fieldValues(field).length;
		...
		case 'group-unique-values': {
			// Only checked on commit (blur, or the change a select or picker fires): while either field is still being typed, a transient match shouldn't flag — only a completed, deliberate duplicate.
			if (!COMMIT_TRIGGERS.has(trigger) || value === '') return null;
	...
	for (const group of model.groups) {
		...
		if (group.kind === 'required-any') {
			...
			for (const name of group.members) {
				results.set(name, { verdict: 'incomplete', code: 'group.required-any', params: {} });
			}
		} else if (group.kind === 'required-together') {
			if (!anyFilled) continue;
			for (const name of group.members) {
				if (ctx.valueOf(name) === '') {
					results.set(name, { verdict: 'incomplete', code: 'group.required-together', params: {} });
				}
			}
		}
	}
