...
export function parseExpression(src) {
	...
	function operand() {
		...
		if (t.type === 'name' && tokens[pos + 1]?.type === '(') {
			// The grammar's one function. A name followed by ( is always a call, and only valid() exists.
...
export function evaluate(node, ctx) {
	switch (node.kind) {
		...
		// valid(name): the named field is answered and its answer passes that field's own validation. A ctx without verdicts treats validity as true, leaving only the answered half.
...
function resolve(node, ctx) {
	...
	return { value: evaluate(node, ctx) ? 'true' : 'false', type: null };
}
...
function compare(node, ctx) {
	...
	// The same type-precedence chain the cross-field rules use: calendar types chronologically, native time as time of day, ordered fs types in their own order, numeric otherwise. A literal operand takes the other side's reading.
...
// Three-valued evaluation for constraint verdicts: 'satisfied' (true now), 'possible' (false now, appending characters could make it true), or 'dead-end' (false now, no continuation can rescue it). Only == can prove a dead-end — two values where neither is a prefix of the other can never grow equal. != and the ordering operators are always repairable, so their false is 'possible'. Negation never claims a dead-end: a satisfied operand can usually be edited false, so its negation stays 'possible'.
...
// Adapts the engine's field context to the shape an expression evaluates against.
export function expressionCtx(ctx) {
	return { get: ctx.valueOf, typeOf: ctx.typeOf, valid: ctx.isFieldValid };
}

// The shared compilation entry: one compile per distinct source, and a malformed expression is reported once and degrades to an inert form — no dependencies, always satisfied, relevant — so an author typo costs that one expression instead of throwing mid-init and taking the form's validation with it.
const compiled = new Map();
export function compiledExpression(src) {
	if (!compiled.has(src)) {
		try {
			compiled.set(src, compileExpression(src));
		} catch (error) {
			console.error(`FormSanity: malformed expression ${JSON.stringify(src)} is ignored — ${error.message}`);
			compiled.set(src, { node: null, deps: [], evaluate: () => true, verdict: () => 'satisfied', error });
		}
	}
