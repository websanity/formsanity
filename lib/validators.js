...
function creditCard(value, param) {
	// ?? rather than a default parameter: an absent attribute arrives as null, which a default parameter would pass through.
...
// Reads a duration as total minutes: either H:MM (single-digit minutes allowed, canonicalization pads them) or bare minutes. Returns null for anything that isn't a complete duration — the min/max comparisons in rules.js share this reading, so bounds see the same values validation accepts.
...
function duration(value) {
	...
	// A zero duration is no duration. It is a dead end when no appended character can make it non-zero: both minute digits typed, or a bare-minutes form at its four digits. A bare "0" or "0:0" can still grow into "0:30".
	if (minutes === 0) return /:[0-5]\d$/.test(value) || /^\d{4}$/.test(value) ? 'invalid' : 'incomplete';
	return 'valid';
}

// Reads a dollar amount as a number, ignoring $ and comma dressing. Shared with the min/max comparisons in rules.js for the same reason as durations.
export function dollarAmount(value) {
	const cleaned = value.replace(/[$,]/g, '');
	return /^\d+(\.\d{0,2})?$/.test(cleaned) ? Number(cleaned) : null;
}

// A time value as seconds since midnight, or null when it isn't one. The hour may omit its leading zero: control values are always padded, but authored literals write '9:30' as naturally as '09:30'.
...
// The fs types with an ordering of their own, and how each reads a value as a comparable number. Rule bounds, cross-field comparisons, and expression operators all order these types through this one table.
...
// Each canonicalizer rewrites an already-valid value into the type's one standard format — the widened acceptance above exists so these have something to fix. Only ever applied to a valid value, so slicing by position is safe.
