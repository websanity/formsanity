...
export function createRelevanceEngine(model, ctx) {
	...
	// Which regions contain each field: a field's effective relevance is its
	// own expression AND every containing region's, conjoined.
	...
	// The row toggles on the field's own expression alone; a region hides or
	// grays its own element, which covers everything inside it. Disabling
	// follows the effective state, so a control inside an irrelevant region
	// is disabled no matter what its own expression says.
	...
	function refresh(changedName) {
		...
		for (const region of hitRegions) {
			region.on = region.compiled.evaluate(evalCtx());
			applyRegion(region);
			for (const member of region.members) touched.add(member);
		}
		for (const name of touched) {
			if (compiled.has(name)) relevant.set(name, compiled.get(name).evaluate(evalCtx()));
