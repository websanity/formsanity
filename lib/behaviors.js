...
function wireCaps(form) {
	for (const control of form.querySelectorAll('[data-fs-prefix], [data-fs-suffix], input[data-fs-reveal], input[type="file"], input[type="date"], input[type="time"], input[type="datetime-local"], input[list]')) {
		...
		if (suffix) {
			...
			// A suggestions cap: showPicker opens the datalist dropdown where the engine supports it, and focusing the field is the fallback.
			...
			// A glyph cap that opens the native picker. Where the engine has no popup for the type (Firefox time), showPicker throws and the cap falls back to focusing the control.
			const picker = cap('fs-suffix', '');
