...
function contribution(el) {
	if (el.matches('option')) {
		...
		if (!select || select.disabled || el.disabled || !el.selected) return 0;
...
// An fs-cols group with no authored fs-col-start balances at its midpoint: the engine adds the same class to the middle row, and the second column starts there. Count-based, computed once at init — rows later hidden by relevance can leave the columns visually uneven. A stacked fs-cols group pairs whole rows in reading order, so it has no columns to balance.
function wireColumnBalance(form) {
	const stackedForm = form.classList.contains('fs-stacked');
	for (const group of form.querySelectorAll('fieldset:not(.fs-toggles) > ul.fs-cols')) {
		if (group.classList.contains('fs-stacked') || (stackedForm && !group.classList.contains('fs-inline'))) continue;
