...
export function attachSubmit(form, state) {
	...
	form.addEventListener('submit', (event) => {
		...
		submit(form, state).finally(() => {
			...
			// A rejection may have marked fields invalid while the control was
			// locked for the flight; the recount closes the gate again if so.
			state.recount?.();
		});
	});
}
...
function validateAll(state) {
	...
	for (const controller of state.controllers.values()) {
		...
		allValid = false;
		// Requiredness never bubbles, even here: the asterisk and the status
		// line are its whole voice. Only wrong answers present.
