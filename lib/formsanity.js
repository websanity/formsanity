...
import { compiledExpression } from './expression.js';
...
function setup(form) {
	...
	let scheduled = false;
	const ctx = { model, valueOf, typeOf, labelOf, onChange, isFieldValid, applyRowState: applyAggregateRowState };
	...
	// valid() in a relevance expression reads verdicts, and the first relevance pass ran before any verdict existed — so settle to a fixed point: each relevance pass reads the latest verdicts, each verdict pass reads the latest relevance, until relevance stops moving. The first verdict pass here is unconditional because verdict-reading rules need it even when relevance never moves: a constraint skips judgment until its referenced fields are valid, which the first controller pass could not yet see. The cap only guards a cyclic model from oscillating forever.
	for (let round = 0; round < 10; round++) {
		const moved = relevance.refresh();
		for (const controller of controllers.values()) {
			controller.refresh();
		}
		if (!moved) break;
	...
	// change fires only on a commit (blur, Enter, or a native picker selection), never per keystroke, so it is the moment a valid value gets rewritten to its canonical format — before the refresh, so bounds and dependent fields read the rewritten value. The commit also carries its own trigger: a picker selection must present errors like a blur would, because no blur ever follows it.
	form.addEventListener('change', (event) => {
		canonicalizeControl(event.target);
		refreshFor(event.target, 'change');
	});
	form.addEventListener('focusout', (event) => refreshFor(event.target, 'blur'), true);

	// Registered after the delegated listeners above so any init-time programmatic dispatches from behaviors (e.g. an amount total that is a real form control) reach the engine's own input handling immediately, not just on the user's next interaction.
	...
	attachSubmit(form, {
		...
		relevance,
		region,
		recount: () => updateFormState({ controllers: [...controllers.values()], form, relevance, region })
	});
	...
	function refreshFor(target, trigger) {
		...
		// The changed field's own verdict settles first so a dependent's valid() reads the new verdict, not last event's.
	...
	function typeOf(name) {
		...
		if (first.type === 'date' || first.type === 'datetime-local' || first.type === 'time') return first.type;
		return field.type;
	}
	...
	function isFieldValid(name) {
		return controllers.get(name)?.verdict === 'valid';
	}
	...
	function buildDependents() {
		...
		for (const field of model.fields.values()) {
			for (const rule of field.rules) {
				if (rule.kind !== 'constraint' && rule.kind !== 'required') continue;
				for (const dep of compiledExpression(rule.param).deps) {
		...
		for (const field of model.fields.values()) {
			for (const own of field.relevance) {
				if (!own) continue;
				for (const dep of compiledExpression(own.expr).deps) {
					if (!map.has(dep)) map.set(dep, new Set());
					map.get(dep).add(field.name);
				}
			}
		}
		// An option's expression deps re-verify the field of its select: the selection can fall back when an option leaves.
		for (const entry of model.options ?? []) {
			if (!entry.select.name) continue;
			for (const dep of compiledExpression(entry.expr).deps) {
				if (!map.has(dep)) map.set(dep, new Set());
				map.get(dep).add(entry.select.name);
			}
		}
		// A region's expression deps re-verify every field the region holds.
		for (const region of model.regions ?? []) {
			for (const dep of compiledExpression(region.expr).deps) {
				for (const member of region.members) {
					if (!map.has(dep)) map.set(dep, new Set());
					map.get(dep).add(member);
				}
			}
		}
		...
		for (const field of model.fields.values()) {
			for (const rule of field.rules) {
				if (rule.kind !== 'group-unique-values') continue;
	...
	// A compound field pairs two fields on one shared row (e.g. two inputs under one aria-labelledby label). Each field's own refresh only knows its own verdict, so naively calling applyRowState per-field is "last writer wins" — a sibling settling back to valid would blank out the row's still-invalid state. This index lets applyAggregateRowState look across the whole row and keep the worst verdict displayed.
...
function updateFormState(state) {
	...
	// The missing line counts unanswered obligations, the invalid line counts wrong answers — including a format left half typed, which is a wrong answer rather than an unanswered question.
	let missing = 0;
	let wrong = 0;
