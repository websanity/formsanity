...
function setup(form) {
	...
	// relevance pass ran before any verdict existed — so settle relevance
	// against the initial verdicts and re-verify whatever it toggled. This
	// is what lets a prefilled valid field enable its dependents at load.
	relevance.refresh();
	for (const controller of controllers.values()) {
		controller.refresh();
	}
