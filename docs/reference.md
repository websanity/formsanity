...
Selects how this field, member, option, or region presents while irrelevant. `hidden`, the default, removes it. `disabled` keeps it in place, dimmed. Goes where `data-fs-relevant` goes, on the element that carries the expression.
