...
// A row may host more than one field (a compound field pair sharing one label), so bubbles are tagged with their owning field name — otherwise a sibling field's showBubble/clearBubble would find and stomp this field's bubble via a bare '.fs-error' lookup on the shared host.
function bubbleSelector(field) {
	return `.fs-error[data-fs-field="${field.name}"]`;
}

// aria-describedby is a token list a field may share with author-supplied hints (e.g. a static "we'll never share this" note). addDescribedBy and removeDescribedBy add/remove only this bubble's own id so an author's other tokens survive an error show/clear cycle intact.
...
export function showBubble(field, message) {
	...
	if (!bubble) {
		...
		// On the field's own row the bubble sits immediately below its control, above any author hint. Other hosts — an errorTo target, a choice group's fieldset — take it at the end.
		if (host && host === field.row) {
			...
			let anchor = control;
			while (anchor.parentElement !== host && anchor.parentElement.matches('.fs-caps, .fs-compound, label')) {
