...
- [Member Relevance](../specs/vocabulary.md#member-relevance) — A submitted value that names an irrelevant member or option is a `relevance` failure. A group with some conditional members reads its relevant members only when an expression names it. A value that names no member of a choice group is the same failure.
