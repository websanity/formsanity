...
export function checkRule(rule, field, ctx, trigger) {
	...
	switch (rule.kind) {
		case 'equals-field': {
			const target = ctx.valueOf(rule.param);
			if (value === target) return null;
			if (value === '') return null;
			const verdict = target.startsWith(value) ? 'incomplete' : 'invalid';
			return { verdict, code: 'equals-field', params: { label: ctx.labelOf(rule.param) } };
		}
		case 'not-equals-field': {
			if (value === '') return null;
			// The param is one or more field names; matching any of them is
			// the violation, and the message names the field it collided with.
			for (const name of rule.param.split(/\s+/).filter(Boolean)) {
				if (ctx.valueOf(name) === value) {
					return { verdict: 'incomplete', code: 'not-equals-field', params: { label: ctx.labelOf(name) } };
				}
			}
			return null;
		}
		...
		case 'less-than-field': {
			...
			// Either operand's type decides how the pair compares: the
			// calendar types chronologically, native time as time of day,
			// the ordered fs types in their own order, everything else
			// numerically. The .date code suffix rides the first two so the
			// message can say after/before rather than greater/less.
			const types = [ctx.typeOf(field.name), ctx.typeOf(rule.param)];
			const chrono = types.some((t) => t === 'date' || t === 'datetime-local');
			const clock = !chrono && types.includes('time');
			const ordered = ORDERED_TYPES[types[0]] ?? ORDERED_TYPES[types[1]];
			let a, b;
			if (chrono) [a, b] = [Date.parse(value), Date.parse(other)];
			else if (clock) [a, b] = [clockSeconds(value), clockSeconds(other)];
			else if (ordered) [a, b] = [ordered(value), ordered(other)];
			else [a, b] = [Number(value), Number(other)];
			if (a === null || b === null || Number.isNaN(a) || Number.isNaN(b)) return null;
			const ok = rule.kind === 'greater-than-field' ? a > b : a < b;
			if (ok) return null;
			const code = chrono || clock ? `${rule.kind}.date` : rule.kind;
		...
		case 'constraint': {
			...
			if (compiled.evaluate({ get: ctx.valueOf, typeOf: ctx.typeOf })) return null;
			return { verdict: 'incomplete', code: 'constraint', params: { message: rule.message } };
		}
		case 'min-time':
		case 'max-time': {
			if (value === '') return null;
