...
export function createRelevanceEngine(model, ctx) {
	...
	// Returns whether any region, option, or control actually toggled, so init can settle relevance and verdicts to a fixed point. A select whose selection fell back dispatches change after the pass, so its dependents refresh through the ordinary path.
