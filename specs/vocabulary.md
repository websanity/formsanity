...
`<`, `<=`, `>`, and `>=` pick their comparison from the operands' types with the same precedence chain the cross-field ordering rules use: **chronologically** when either operand is a `date` or `datetime-local` field, as **time of day** when either is a native `time` field, in **the type's own order** when either carries an ordered fs type (`duration`, `us-dollar`), and **numerically** otherwise. A literal operand takes the other side's reading. A time-of-day literal may omit the hour's leading zero — `'9:30'` and `'09:30'` name the same instant; control values themselves are always padded. A comparison MUST evaluate to false when either operand is empty or blank, and when either operand fails to parse as the chosen kind. This makes `qty > 3` false for an unanswered `qty` rather than an error.
