...
// data-fs-clear-on-change="source-name …": a dependent answer (a confirm
// field, a state select under a country select) empties whenever any named
// source field's value changes — a stale dependent answer is worse than an
// empty one. The non-empty guard is also the loop-breaker for mutual pairs.
function wireClearOnChange(form) {
	for (const control of form.querySelectorAll('[data-fs-clear-on-change]')) {
		const sources = control.getAttribute('data-fs-clear-on-change').split(/\s+/).filter(Boolean);
		form.addEventListener('input', (event) => {
			if (event.target === control || !sources.includes(event.target.name)) return;
			const checkable = control.type === 'checkbox' || control.type === 'radio';
			if (checkable ? !control.checked : control.value === '') return;
...
function wireCopyTo(form, model) {
	...
	form.addEventListener('input', (event) => {
		...
		// The state-equality checks double as the cycle guard: once the
		// target already holds the state being written, nothing dispatches
		// and a copy loop goes quiet.
		if (first.type === 'radio') {
			// A radio target checks its member matching the source's value.
			for (const control of field.controls) {
				const want = control.value === event.target.value;
				if (control.checked !== want) {
					control.checked = want;
					if (want) dispatchInput(control);
				}
			}
		} else if (first.type === 'checkbox') {
			// A checkbox target mirrors the checked state of the source's
			// own value, leaving its other members alone.
			const match = field.controls.find((control) => control.value === event.target.value);
			if (match && match.checked !== event.target.checked) {
				match.checked = event.target.checked;
				dispatchInput(match);
			}
		} else {
			if (first.value === event.target.value) return;
...
// One term's contribution. Bare data-fs-amount reads the control's own value
// as the amount. With a value, the attribute is the price, separate from the
// answer: a numeric answer multiplies it (unit pricing), any other answer —
// a checked choice, a chosen file, a filled field — charges it once, and no
// answer charges nothing. An amount may be negative: a discount. An option
// carrying the attribute charges flat while selected.
...
// A cols group with no authored col-break balances at its midpoint, as v1
// did: the engine marks the middle row and the stylesheet's split machinery
// treats it like a manual break. Count-based, computed once at init — rows
// later hidden by relevance can leave the columns visually uneven, as in v1.
function wireColumnBalance(form) {
	for (const group of form.querySelectorAll('fieldset:not(.toggle-list) > ul.cols')) {
		if (group.querySelector(':scope > li.col-break')) continue;
		const rows = [...group.children].filter((child) => child.matches('li:not(.block)'));
		if (rows.length < 2) continue;
		rows[Math.ceil(rows.length / 2)].classList.add('fs-col-break');
	}
}
...
function wireDeselect(form) {
	const keepsAnswer = (radio) => {
		const members = [...form.querySelectorAll('input[type="radio"]')].filter((r) => r.name === radio.name);
		return members.some((r) => r.required) && members.some((r) => r.defaultChecked);
	};
	...
	form.addEventListener('click', (event) => {
		...
		if (pending?.radio === event.target && pending.wasChecked && !keepsAnswer(event.target)) {
...
function wireCaps(form) {
	for (const control of form.querySelectorAll('[data-fs-prefix], [data-fs-suffix], input[data-fs-reveal], input[type="file"], input[type="date"], input[type="time"], input[type="datetime-local"], input[list]')) {
		...
		if (suffix) {
			...
			// A suggestions cap: showPicker opens the datalist dropdown where
			// the engine supports it, and focusing the field is the fallback.
