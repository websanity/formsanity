...
export function showBubble(field, message) {
	...
	if (!bubble) {
		...
		// control (or the caps wrapper holding it), above any author hint.
		// Other hosts — an errorTo target, a choice group's fieldset —
		// take it at the end.
		if (host && host === field.row) {
			const control = field.controls[field.controls.length - 1];
			(control.closest('.fs-caps') ?? control).after(bubble);
		} else {
